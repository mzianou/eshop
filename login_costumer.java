...
public class login_costumer extends HttpServlet{
	public String cosid;
	Connection con;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(!allowUser(name,password,request)){
			...
			out.println("<p align=center>Try again</p><center><a href=\"login.html\"><br> <b>Here</b> </a></center><br>");
			out.println("<hr><p align=center>© 2021 Zianou, All Rights Reserved</p>");
			out.println("</body></html>");
		} else{ 
			HttpSession session=request.getSession();
			session.setAttribute("login.isDone",name);
			if ( cosid != null ){
				session.setAttribute("curUser",cosid);
			}
			String target=(String) session.getAttribute("login_costumer.target");
			if(target!=null) {
				response.sendRedirect(target);
				return;
			}
			response.sendRedirect("arxikh_sel.jsp");
		}
	}
